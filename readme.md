...
* The app works great in Chrome and Firefox (latest versions). It struggles with Internet Explorer. IE doesn't recognize some of the finer user flow processes or SVG components used by Polymer. It also formats dates differently than Chrome/Firefox/D3, which took a date formatting polyfill to correct. I've fixed what I could, but it's going to be much slower and clunkier when using IE than when using Chrome.
